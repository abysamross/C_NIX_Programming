
register.o:     file format elf64-x86-64
register.o
architecture: i386:x86-64, flags 0x00000011:
HAS_RELOC, HAS_SYMS
start address 0x0000000000000000

Sections:
Idx Name          Size      VMA               LMA               File off  Algn
  0 .text         00000048  0000000000000000  0000000000000000  00000040  2**0
                  CONTENTS, ALLOC, LOAD, RELOC, READONLY, CODE
  1 .data         00000000  0000000000000000  0000000000000000  00000088  2**0
                  CONTENTS, ALLOC, LOAD, DATA
  2 .bss          00000000  0000000000000000  0000000000000000  00000088  2**0
                  ALLOC
  3 .comment      0000002c  0000000000000000  0000000000000000  00000088  2**0
                  CONTENTS, READONLY
  4 .note.GNU-stack 00000000  0000000000000000  0000000000000000  000000b4  2**0
                  CONTENTS, READONLY
  5 .eh_frame     00000058  0000000000000000  0000000000000000  000000b8  2**3
                  CONTENTS, ALLOC, LOAD, RELOC, READONLY, DATA
SYMBOL TABLE:
0000000000000000 l    df *ABS*	0000000000000000 register.c
0000000000000000 l    d  .text	0000000000000000 .text
0000000000000000 l    d  .data	0000000000000000 .data
0000000000000000 l    d  .bss	0000000000000000 .bss
0000000000000000 l    d  .note.GNU-stack	0000000000000000 .note.GNU-stack
0000000000000000 l    d  .eh_frame	0000000000000000 .eh_frame
0000000000000000 l    d  .comment	0000000000000000 .comment
0000000000000000 g     F .text	000000000000002d func
000000000000002d g     F .text	000000000000001b main



Disassembly of section .text:

0000000000000000 <func>:
   0:	55                   	push   %rbp
   1:	48 89 e5             	mov    %rsp,%rbp
   4:	89 7d ec             	mov    %edi,-0x14(%rbp)
   7:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%rbp)
   e:	c7 45 f8 80 00 00 00 	movl   $0x80,-0x8(%rbp)
  15:	83 6d f8 01          	subl   $0x1,-0x8(%rbp)
  19:	8b 45 fc             	mov    -0x4(%rbp),%eax
  1c:	8b 55 ec             	mov    -0x14(%rbp),%edx
  1f:	01 c2                	add    %eax,%edx
  21:	8b 45 f4             	mov    -0xc(%rbp),%eax
  24:	01 c2                	add    %eax,%edx
  26:	8b 45 f8             	mov    -0x8(%rbp),%eax
  29:	01 d0                	add    %edx,%eax
  2b:	5d                   	pop    %rbp
  2c:	c3                   	retq   

000000000000002d <main>:
  2d:	55                   	push   %rbp
  2e:	48 89 e5             	mov    %rsp,%rbp
  31:	48 83 ec 10          	sub    $0x10,%rsp
  35:	c7 45 fc ff ff ff ff 	movl   $0xffffffff,-0x4(%rbp)
  3c:	8b 45 fc             	mov    -0x4(%rbp),%eax
  3f:	89 c7                	mov    %eax,%edi
  41:	e8 00 00 00 00       	callq  46 <main+0x19>
			42: R_X86_64_PC32	func-0x4
  46:	c9                   	leaveq 
  47:	c3                   	retq   
